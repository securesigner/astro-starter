 ...
 * Dropdown select menu built on Radix UI primitives.
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    className={cn(
      ...
      "flex h-9 w-full items-center justify-between whitespace-nowrap rounded-md border px-3 py-1 text-base shadow-sm transition-colors md:text-sm",
  ...
  >
    ...
    <SelectPrimitive.Icon asChild>
      <ChevronDown className="h-4 w-4 opacity-60 text-gray-500 dark:text-gray-400" />
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn("px-2 py-1.5 text-sm font-semibold text-gray-500 dark:text-gray-400", className)}
    {...props}
  />
))
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      // Base styling with touch-friendly sizing (min 44px touch target)
      "relative flex w-full cursor-pointer select-none items-center rounded-sm py-2.5 pl-3 pr-8 text-sm outline-none",
      // Hover and focus states with emerald accent (brand-consistent)
      "focus:bg-emerald-500/20 focus:text-gray-900 dark:focus:text-gray-100 data-[highlighted]:bg-emerald-500/20 data-[highlighted]:text-gray-900 dark:data-[highlighted]:text-gray-100",
      // Text styling
